...
function calculateLoan() {
  ...
  document.getElementById(
    "loanResult"
  ).innerText = `Pago mensual: $${monthlyPayment.toFixed(2)}`;
...
function updateLoanHistory() {
  ...
  loanHistory.forEach((calculation, index) => {
    ...
    listItem.innerHTML = `
                Monto: $${calculation.loanAmount}, Tasa: ${(
      calculation.interestRate *
      12 *
      100
    ).toFixed(2)}%, Plazo: ${
      calculation.loanTerm / 12
    } años, Pago mensual: $${calculation.monthlyPayment.toFixed(2)}
...
document
  .getElementById("resetHistoryButton")
